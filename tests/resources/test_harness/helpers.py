...
def get_master_device_simulators(simulator_factory):
    ...
    dish_master_sim_7 = simulator_factory.get_or_create_simulator_device(
        SimulatorDeviceType.DISH_DEVICE, sim_number=7
    )
    return (
        csp_master_sim,
        sdp_master_sim,
...
def check_for_device_command_event(
    ...
    while not event_found and elapsed_time < timeout:
        ...
        if assertion_data and assertion_data["attribute_value"][0].endswith(command_name):
